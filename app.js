...
function createBids(){
    // Subtract 1% for fees
    var available = Env.AMOUNT_EUR * 0.99;
    winston.log('info', "Amount available in account: EUR " + available);

    var amountPerOrder = available / Env.ORDER_COUNT;
    winston.log('info', "Amount per buy order: EUR " + amountPerOrder);
        ...
        async.eachSeries(ordersToCreate, function(order, callback){
          ...
          authedClient.buy(order, function(error, response, data){
            ...
            Order(order).save(function(error){
              ...
              if(error){
                winston.log('error', "Failed to create order" + order.id + " with error " + error);
                callback(error);
                return;
              }
